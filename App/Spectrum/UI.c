...
static void Show_Wave(Spectrum_Sample *sample) {
    ...
    uint16_t last_i = 0, last_val;
    for (uint16_t i = 0; i < SPECTRUM_X_NUM; i++) { // 以示波器的横轴每个像素点为循环主元
        if ((uint16_t) j != j_int) { // j累计到一个新整数
            j_int++;

            float voltage = sample->data.fft[j_int] / (SPECTRUM_SAMPLE_NUM / 2.0f); // FFT把数据放大了(n/2)倍，n为FFT长度
            ...
            if (j_int != 0)
                OLED_DrawLine(SPECTRUM_X_MIN + last_i, last_val, SPECTRUM_X_MIN + i, val, 1);
            last_i = i;
            last_val = val;
        }
        j += ratio;
...
static void Show_Information(Spectrum_Sample *sample) {
    ...
    // 显示测量参数
    OLED_ShowString(0, 104, "Bias: ", 8, 1);
    floatToStr(str_buf, sample->bias, 1);
    strcat(str_buf, "V");
    OLED_ShowString(36, 104, str_buf, 8, 1);

    OLED_ShowString(0, 116, "Max: ", 8, 1);
    floatToStr(str_buf, sample->max, 1);
    strcat(str_buf, "V");
    OLED_ShowString(30, 116, str_buf, 8, 1);

    len = floatToStr(str_buf, sample->freq, 0) + 2;
    strcat(str_buf, "Hz");
    OLED_ShowString(127 - len * 6, 116, str_buf, 8, 1);
}
